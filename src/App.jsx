...
function MetCard({ label, value, color, highlight }) {
  return (
    <div style={{
      ...
      border: highlight ? "none" : "1px solid #E8E0D0",
...
function Input({ style, ...props }) {
  return (
    <input {...props} style={{
      width: "100%", padding: "10px 12px", borderRadius: 8, border: "1.5px solid #E8E0D0",
      fontSize: 14, color: "#1A2E1A", background: "#FAFAF7", outline: "none",
      boxSizing: "border-box", transition: "border-color 0.15s", ...style
    }}
    onFocus={e => e.target.style.borderColor = "#5B8C3E"}
...
function SectionLabel({ children }) {
  return (
    <div style={{ fontSize: 11, color: "#6B6B5A", fontWeight: 700, marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.08em" }}>
      {children}
    </div>
  );
}
...
function ProductForm({ initial, onSave, onCancel, existingIds = [] }) {
  ...
  const handleSave = () => {
    ...
    const id = isNew
      ? (() => {
          let base = slugify(draft.name) || "product";
    ...
    <div style={{ background: "#F5F0E8", borderRadius: 10, padding: 14, marginBottom: 10 }}>
      <div style={{ display: "grid", gridTemplateColumns: "56px 1fr", gap: 10, marginBottom: 12 }}>
        <div>
          ...
          <Input value={draft.emoji} onChange={e => set("emoji", e.target.value)} style={{ textAlign: "center", fontSize: 20, padding: "8px 4px" }} maxLength={2} />
        </div>
        <div>
          <FieldLabel>Product name</FieldLabel>
          <Input value={draft.name} onChange={e => set("name", e.target.value)} placeholder="e.g. Garlic Dill" />
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
        <div>
          <FieldLabel>Cost per jar ($)</FieldLabel>
          <Input type="number" step="0.01" min="0" value={draft.costPerUnit} onChange={e => set("costPerUnit", e.target.value)} placeholder="2.50" />
        </div>
        <div>
          <FieldLabel>Default sale price ($)</FieldLabel>
          <Input type="number" step="0.01" min="0" value={draft.defaultPrice} onChange={e => set("defaultPrice", e.target.value)} placeholder="8.00" />
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={handleSave} disabled={!valid} style={{
          ...
          background: valid ? "#5B8C3E" : "#C8C0B0", color: "#fff", fontWeight: 700,
...
export default function PickleTracker() {
  ...
  const persist = (s, p) => {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ sales: s, products: p })); } catch {}
  };
  ...
  const handleLogSale = () => {
    ...
    const sale = {
      ...
      quantity: Number(saleQty), pricePerUnit: Number(salePrice),
      totalPrice: Number(salePrice) * Number(saleQty),
      costPerUnit: product.costPerUnit, totalCost: product.costPerUnit * Number(saleQty),
      customer: saleCustomer, note: saleNote,
    };
    const ns = [sale, ...sales];
    setSales(ns); persist(ns, products);
    ...
    <div style={{ fontFamily:"'Inter', system-ui, sans-serif", background:"#F5F0E8", minHeight:"100vh" }}>
      ...
      <div style={{ background:"#fff", borderBottom:"1px solid #E8E0D0", display:"flex" }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            ...
            borderBottom: tab===t.id ? "3px solid #5B8C3E" : "3px solid transparent",
            color: tab===t.id ? "#5B8C3E" : "#6B6B5A",
      ...
      <div style={{ padding:"16px", maxWidth:560, margin:"0 auto" }}>
        ...
        {tab === "dashboard" && (<>
          ...
          <SectionLabel>Monthly Revenue & Profit ({new Date().getFullYear()})</SectionLabel>
          <div style={{ background:"#fff", borderRadius:12, padding:"16px 8px 10px", boxShadow:"0 1px 4px rgba(0,0,0,0.07)", marginBottom:22 }}>
            <ResponsiveContainer width="100%" height={190}>
              <BarChart data={monthlyData} barGap={3}>
                <CartesianGrid strokeDasharray="3 3" stroke="#F0EAE0" />
                <XAxis dataKey="month" tick={{ fontSize:10, fill:"#6B6B5A" }} />
                <YAxis tick={{ fontSize:10, fill:"#6B6B5A" }} tickFormatter={v=>`$${v}`} width={40} />
                <Tooltip formatter={(v,n)=>[`$${v.toFixed(2)}`,n==="revenue"?"Revenue":"Profit"]} />
                <Legend formatter={v=>v==="revenue"?"Revenue":"Profit"} wrapperStyle={{ fontSize:12 }} />
                <Bar dataKey="revenue" fill="#D4A843" radius={[4,4,0,0]} />
                <Bar dataKey="profit"  fill="#5B8C3E" radius={[4,4,0,0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div style={{ background:"#fff", borderRadius:12, padding:"4px 14px", boxShadow:"0 1px 4px rgba(0,0,0,0.07)" }}>
            {sales.length===0
              ? <div style={{ textAlign:"center", color:"#9A9A8A", padding:"28px 0", fontSize:14 }}>No sales yet — log your first jar! 🥒</div>
              : sales.slice(0,5).map(s=><SaleRow key={s.id} sale={s}/>)
            }
          </div>
          {sales.length>5 && (
            <button onClick={()=>setTab("history")} style={{ width:"100%", marginTop:8, padding:"11px", background:"none", border:"1.5px dashed #C8C0B0", borderRadius:10, color:"#5B8C3E", fontSize:13, cursor:"pointer", fontWeight:700 }}>
        ...
        {tab === "log" && (
          <div style={{ background:"#fff", borderRadius:14, padding:20, boxShadow:"0 1px 4px rgba(0,0,0,0.07)" }}>
            ...
            {saleSuccess && (
              <div style={{ background:"#E8F5E0", border:"1.5px solid #5B8C3E", borderRadius:8, padding:"10px 14px", color:"#3D6B2F", fontSize:13, marginBottom:16, fontWeight:700 }}>✅ Sale logged successfully!</div>
            )}
            {products.length===0 ? (
              <div style={{ textAlign:"center", color:"#9A9A8A", padding:"24px 0", fontSize:13 }}>
                No products yet — <button onClick={()=>setTab("products")} style={{ color:"#5B8C3E", background:"none", border:"none", cursor:"pointer", fontWeight:700, fontSize:13 }}>add one in Products</button>
              </div>
            ) : (<>
        ...
        {tab === "history" && (<>
          ...
          {filteredSales.length===0
            ? <div style={{ textAlign:"center", color:"#9A9A8A", padding:"48px 0", fontSize:14 }}>No sales to show.</div>
            : filteredSales.map(s=>(
        ...
        {tab === "products" && (<>
          ...
          {showAddForm
            ...
            : <button onClick={()=>{ setShowAddForm(true); setEditProductId(null); setConfirmDeleteId(null); }}
                style={{ width:"100%", marginBottom:14, padding:"12px", borderRadius:10, border:"2px dashed #5B8C3E", background:"#E8F5E0", color:"#3D6B2F", fontSize:14, fontWeight:700, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
